...
timecourse_item_child = test_df %>% filter(child_adult_group == 'children') %>%
  make_time_sequence_data(time_bin_size = 1, 
                                         predictor_columns = c("condition",
                                                               "novelword"),
                                         aois = c("AVERAGE_IA_1_SAMPLE_COUNT_."),
                                         summarize_by = "participant_number")
...
Add SES to prop looks modeL:
```{r}
fl_looking_ses_lm = lm(prop_correct_looks_but_z_score ~ composite_congruency_effect + composite_switch_effect + flanker_nogo_effect_z_score + age_months_z_score + educ_level_parent1 + prop_correct_looks_so_z_score, data = prop_looks_correct_fl_ses)
summary(fl_looking_ses_lm)

vif(fl_looking_ses_lm)
```

Try without combining RT and acc z-scores:
```{r}
fl_looking_ses_separate_lm = lm(prop_correct_looks_but_z_score ~ flanker_congruency_rt_z_score + flanker_congruency_acc_z_score + flanker_switch_rt_z_score + flanker_switch_acc_z_score + flanker_nogo_effect_z_score + age_months_z_score + educ_level_parent1 + prop_correct_looks_so_z_score, data = prop_looks_correct_fl_ses)
summary(fl_looking_ses_separate_lm)

vif(fl_looking_ses_separate_lm)
```
...
Try separating accuracy and RT costs, without outliers:
```{r}
