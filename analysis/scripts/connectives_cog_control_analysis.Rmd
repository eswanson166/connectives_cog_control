...
Get mean and SD of samples contributed per trial:
```{r}
...
Pull out just arcsin-transformed prop of looks to correct image for adults and children (we will use this for the relationship to Flanker data later):
```{r}
prop_looks_acc_partic_child = window_test_df_partic_acc %>% 
  select(participant_number, condition, age_group, child_adult_group, ArcSin) %>%
  filter(child_adult_group == 'child') %>%
  spread(key = 'condition', value = 'ArcSin') %>%
  rename(arcsin_prop_correct_looks_but = but, arcsin_prop_correct_looks_so = so)
...
```{r}
fl_pointing_lm = lm(prop_correct_points_but_z_score ~ composite_congruency_effect + composite_switch_effect + nogo_cost_z_score + age_months_z_score + prop_correct_points_so_z_score, 
...
```{r}
fl_pointing_separate_lm = lm(prop_correct_points_but_z_score ~ flanker_congruency_rt_z_score + flanker_congruency_errors_z_score + flanker_switch_rt_z_score + flanker_switch_errors_z_score +  nogo_cost_z_score + age_months_z_score + prop_correct_points_so_z_score, 
                             data = pointing_df_participant)
...
```{r}
prop_looks_acc_partic_fl$mean_arcsin_prop_correct_looks_so = mean(prop_looks_acc_partic_fl$arcsin_prop_correct_looks_so)
prop_looks_acc_partic_fl$sd_arcsin_prop_correct_looks_so = sd(prop_looks_acc_partic_fl$arcsin_prop_correct_looks_so)

prop_looks_acc_partic_fl = prop_looks_acc_partic_fl %>% 
  mutate(arcsin_prop_correct_looks_so_z_score = ((arcsin_prop_correct_looks_so - mean_arcsin_prop_correct_looks_so)/sd_arcsin_prop_correct_looks_so))
```

Calculate z-score for proportion of looks to correct image on but trials:
```{r}
prop_looks_acc_partic_fl$mean_arcsin_prop_correct_looks_but = mean(prop_looks_acc_partic_fl$arcsin_prop_correct_looks_but)
prop_looks_acc_partic_fl$sd_arcsin_prop_correct_looks_but = sd(prop_looks_acc_partic_fl$arcsin_prop_correct_looks_but)

prop_looks_acc_partic_fl = prop_looks_acc_partic_fl %>% 
  mutate(arcsin_prop_correct_looks_but_z_score = ((arcsin_prop_correct_looks_but - mean_arcsin_prop_correct_looks_but)/sd_arcsin_prop_correct_looks_but))
```

Run a regression to test whether Flanker performance predicts looking performance on but trials:
```{r}
fl_looking_lm = lm(arcsin_prop_correct_looks_but_z_score ~ composite_congruency_effect + composite_switch_effect + nogo_cost_z_score + age_months_z_score + arcsin_prop_correct_looks_so_z_score, data = prop_looks_acc_partic_fl)
summary(fl_looking_lm)
```

Try without combining RT and acc z-scores:
```{r}
fl_looking_separate_lm = lm(arcsin_prop_correct_looks_but_z_score ~ flanker_congruency_rt_z_score + flanker_congruency_errors_z_score + flanker_switch_rt_z_score + flanker_switch_errors_z_score +  nogo_cost_z_score + age_months_z_score + arcsin_prop_correct_looks_so_z_score, data = prop_looks_acc_partic_fl)
summary(fl_looking_separate_lm)
```

Try without performance on so trials as a predictor, with composites:
```{r}
fl_looking_no_so_lm = lm(arcsin_prop_correct_looks_but_z_score ~ composite_congruency_effect + composite_switch_effect + nogo_cost_z_score + age_months_z_score, data = prop_looks_acc_partic_fl)
summary(fl_looking_no_so_lm)
```

Try without combining RT and acc z-scores and without performance on so trials:
```{r}
fl_looking_separate_no_so_lm = lm(arcsin_prop_correct_looks_but_z_score ~ flanker_congruency_rt_z_score + flanker_congruency_errors_z_score + flanker_switch_rt_z_score + flanker_switch_errors_z_score +  nogo_cost_z_score + age_months_z_score, data = prop_looks_acc_partic_fl)
